...
import aiohttp
from gold_rs import AugSchemeMPL, G2Element, PrivateKey
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: WSGoldConnection
    ) -> None:
        """
        ...
        for sp in sps:
            ...
            if p2_singleton_puzzle_hash is not None:
                ...
                required_iters = calculate_iterations_quality(
                    ...
                    Decimal("1.5"),  # FIXME handle staking in pool protocol
                    new_proof_of_space.sp_hash,
                )
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Gold Blockchain v.{__version__}"},
                        ) as resp:
                            if not resp.ok:
    ...
    async def new_signage_point(
            self, new_signage_point: farmer_protocol.NewSignagePoint, peer: WSGoldConnection
    ) -> None:
        ...
        try:
            ...
            response = await peer.call_api(FullNodeAPI.request_stakings, farmer_protocol.RequestStakings(
                public_keys=self.farmer.get_public_keys(), height=None, blocks=None
            ))
            if response is None:
                return
    ...
    @api_request(peer_required=True)
    async def farming_info(self, request: farmer_protocol.FarmingInfo, peer: WSGoldConnection) -> None:
        self.farmer.state_changed(
            "new_farming_info",
            {
                "farming_info": {
    ...
    @api_request(peer_required=True)
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: WSGoldConnection) -> None:
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")

    @api_request(peer_required=True)
    async def plot_sync_start(self, message: PlotSyncStart, peer: WSGoldConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request(peer_required=True)
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: WSGoldConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request(peer_required=True)
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: WSGoldConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request(peer_required=True)
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: WSGoldConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request(peer_required=True)
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: WSGoldConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request(peer_required=True)
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: WSGoldConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request(peer_required=True)
    async def plot_sync_done(self, message: PlotSyncDone, peer: WSGoldConnection) -> None:
