...
from chia.server.server import GoldServer, ssl_context_for_root
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'gold keys generate' or open the UI."
        try:
        ...
        if "gl_target_address" not in self.config:
            self.config = config["farmer"]
        if "gl_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "gl_target_address" not in self.config or "gl_target_address" not in self.pool_config:
            log.debug("gl_target_address missing in the config")
            return False
