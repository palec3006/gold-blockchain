...
from chia.plotters.chiapos import get_chiapos_install_info, plot_gold
...
def call_plotters(root_path: Path, args):
    ...
    plotters = argparse.ArgumentParser("gold plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, chia_plotter_options, "chiapos", "Create a plot with the default gold plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'gold plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'gold plotters bladebit2' was integrated to 'gold plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        show_plotters_version(gold_root_path)
