...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSGoldConnection) -> CoinState:
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            gold_tx = await self.create_tandem_gl_tx(
                fee, AssertAnnouncement(True, asserted_origin_id=current_coin.name(), asserted_msg=b"$"), tx_config
            )
            assert gold_tx.spend_bundle is not None
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, gold_tx.spend_bundle])
            ...
            txs: List[TransactionRecord] = [dl_tx, gold_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            [gold_tx] = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
                tx_config,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, gold_tx.spend_bundle])
                ),
                dataclasses.replace(gold_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSGoldConnection, coin_data: Optional[object]) -> None:
