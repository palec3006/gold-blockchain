...
from gold_rs import PrivateKey
...
from chia.util.config import create_default_gold_config, load_config, save_config
...
def create_config(
    ...
    create_default_gold_config(gold_root)
    create_all_ssl(
        gold_root,
    ...
    # load config
    config = load_config(gold_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "tgl")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(gold_root, "config.yaml", config)
    return config


async def start_simulator(gold_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    started_simulator = await start_simulator_main(True, automated_testing, root_path=gold_root)
...
async def get_full_gold_simulator(
    gold_root: Path,
    ...
    """
    A gold root Path is required.
    The gold root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the gold test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(gold_root)):
        ...
        crt_path = gold_root / config["daemon_ssl"]["private_crt"]
        key_path = gold_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = gold_root / config["private_ssl_ca"]["crt"]
        ca_key_path = gold_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(gold_root, ca_crt_path, ca_key_path, crt_path, key_path)
        async with SignalHandlers.manage() as signal_handlers:
            await ws_server.setup_process_global_state(signal_handlers=signal_handlers)
            async with ws_server.run():
                async for simulator in start_simulator(gold_root, automated_testing):
                    yield simulator, gold_root, config, mnemonic, fingerprint, keychain
