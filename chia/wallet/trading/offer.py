...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting GL
        coins: List[Coin],
    ...
    def _get_offered_coins(self) -> Dict[Optional[bytes32], List[Coin]]:
        ...
        for parent_spend in self._bundle.coin_spends:
            ...
            if puzzle_driver is not None:
                ...
                # It's much easier if the asset is bare GL
                asset_id = None
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "gl" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
