...
from gold_rs import AugSchemeMPL, G1Element, G2Element
from clvm.casts import int_from_bytes, int_to_bytes
from typing_extensions import Unpack

import chia.wallet.singleton
from chia.protocols.wallet_protocol import CoinState
from chia.server.ws_connection import WSGoldConnection
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGoldConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSGoldConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        unsigned_spend_bundle, gold_tx = await self.generate_unsigned_spendbundle(
            payments,
            tx_config,
            fee,
            coins=coins,
        ...
        if gold_tx is not None and gold_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, gold_tx.spend_bundle])
            gold_tx = dataclasses.replace(gold_tx, spend_bundle=None)
            other_tx_removals: Set[Coin] = {removal for removal in gold_tx.removals}
            other_tx_additions: Set[Coin] = {addition for addition in gold_tx.additions}
        else:
        ...
        if gold_tx is not None:
            tx_list.append(gold_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            gold_tx = await self.standard_wallet.create_tandem_gl_tx(
                fee,
                tx_config,
        ...
        return nft_spend_bundle, gold_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'GL'} to offer")
    ...
    async def mint_from_did(
        ...
        gl_coins: Optional[Set[Coin]] = None,
        gl_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an gl spend that funds the transactions and pays fees.
        ...
        :param gl_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param gl_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the gl spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an gl coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if gl_coins is None:
            gl_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        assert len(gl_coins) > 0
        ...
        # Create the gl spend to fund the minting.
        spend_value = sum([coin.amount for coin in gl_coins])
        change: uint64 = uint64(spend_value - total_amount)
        if gl_change_ph is None:
            gl_change_ph = await self.standard_wallet.get_new_puzzlehash()
        gl_payment = Payment(gl_change_ph, change, [gl_change_ph])

        gl_coins_iter = iter(gl_coins)
        gl_coin = next(gl_coins_iter)

        message_list: List[bytes32] = [c.name() for c in gl_coins]
        message_list.append(Coin(gl_coin.name(), gl_payment.puzzle_hash, gl_payment.amount).name())
        message: bytes32 = std_hash(b"".join(message_list))

        gl_extra_conditions: Tuple[Condition, ...] = (
            AssertCoinAnnouncement(asserted_id=did_coin.name(), asserted_msg=message),
        )
        if len(gl_coins) > 1:
            gl_extra_conditions += (CreateCoinAnnouncement(message),)

        solution: Program = self.standard_wallet.make_solution(
            primaries=[gl_payment],
            fee=fee,
            conditions=gl_extra_conditions,
        )
        primary_announcement_hash = AssertCoinAnnouncement(asserted_id=gl_coin.name(), asserted_msg=message).msg_calc
        # connect this coin assertion to the DID announcement
        did_coin_announcement = CreateCoinAnnouncement(message)
        puzzle = await self.standard_wallet.puzzle_for_puzzle_hash(gl_coin.puzzle_hash)
        gl_spends = [make_spend(gl_coin, puzzle, solution)]

        for gl_coin in gl_coins_iter:
            puzzle = await self.standard_wallet.puzzle_for_puzzle_hash(gl_coin.puzzle_hash)
            solution = self.standard_wallet.make_solution(
                primaries=[], conditions=(AssertCoinAnnouncement(primary_announcement_hash),)
            )
            gl_spends.append(make_spend(gl_coin, puzzle, solution))
        gl_spend = SpendBundle(gl_spends, G2Element())
        ...
        total_spend = SpendBundle.aggregate([unsigned_spend_bundle, gl_spend, *eve_spends])

        tx_record: TransactionRecord = dataclasses.replace(eve_txs[0], spend_bundle=total_spend)
        return [tx_record]

    async def mint_from_gl(
        self,
        metadata_list: List[Dict[str, Any]],
        tx_config: TXConfig,
        target_list: Optional[List[bytes32]] = [],
        ...
        """
        Minting NFTs from a single GL spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = gl_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from GL to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the GL spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the GL spend
            ...
            # Annnouncements for eve spend. These are asserted by the gl spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        first = True
        for gl_coin in gl_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(gl_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in gl_coins]
                message_list.append(Coin(gl_coin.name(), gl_payment.puzzle_hash, gl_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[gl_payment] + primaries,
                    fee=fee,
                    conditions=extra_conditions,
                )
                primary_announcement = AssertCoinAnnouncement(asserted_id=gl_coin.name(), asserted_msg=message)
                first = False
            else:
