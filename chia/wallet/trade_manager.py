...
class TradeManager:
    ...
    By default, standard GL is supported but to support other types of assets you must implement certain functions on
    ...
    async def coins_of_interest_farmed(
        self, coin_state: CoinState, fork_height: Optional[uint32], peer: WSGoldConnection
    ) -> None:
        """
    ...
    async def cancel_pending_offers(
        ...
        for trade, cancellation_coins in zip(trade_records, all_cancellation_coins):
            ...
            for coin in cancellation_coins:
                ...
                # This should probably not switch on whether or not we're spending a GL but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, amount in offer_dict.items():
                ...
                # asset_id can either be none if asset is GL or
                ...
                if amount > 0:
                    ...
                    # For the GL wallet also include the fee amount to the coins we use to pay this offer
                ...
                if asset_id is not None and wallet is not None:  # if this asset is not GL
            ...
            # The access of the sorted keys here makes sure we create the GL transaction first to make sure we pay fee
            # with the GL side of the offer and don't create an extra fee transaction in other wallets.
            for id in sorted(coins_to_offer.keys(), key=lambda id: id != 1):
                ...
                # This should probably not switch on whether or not we're spending GL but it has to for now
                if wallet.type() == WalletType.STANDARD_WALLET:
                    [tx] = await wallet.generate_signed_transaction(
                        abs(offer_dict[id]),
                        Offer.ph(),
    ...
    async def check_offer_validity(self, offer: Offer, peer: WSGoldConnection) -> bool:
