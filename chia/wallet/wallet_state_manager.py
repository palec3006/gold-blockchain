...
class WalletStateManager:
    ...
    async def create(
        ...
        self.stake_address = create_puzzlehash_for_pk(master_sk_to_farmer_sk(private_key).get_g1())
    ...
    async def determine_coin_type(
        self, peer: WSGoldConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the GL coin
        ...
        if fee > 0:
            gold_tx = await self.main_wallet.create_tandem_gl_tx(
                fee,
                tx_config,
                extra_conditions=(
            ...
            tx_list.append(dataclasses.replace(gold_tx, spend_bundle=None))
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        gl_spam_amount = self.config.get("gl_spam_amount", 1000000)
        ...
        small_unspent_count = await self.coin_store.count_small_unspent(gl_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < gl_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSGoldConnection) -> Optional[bytes32]:
        # Get minter DID
