...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGoldConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
        ...
        for coin in coin_records:
            ...
            if first:
                ...
                if need_gold_transaction:
                    ...
                    elif regular_gold_to_claim > fee:
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [gold_tx.spend_bundle]
            ...
            [*vc_txs, *([gold_tx] if gold_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(gold_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if gold_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, gold_tx.spend_bundle])
        else:
            gold_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(gold_tx, spend_bundle=None),) if gold_tx is not None else []),
        ]
