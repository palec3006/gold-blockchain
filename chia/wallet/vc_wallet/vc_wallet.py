...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGoldConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            gold_tx = await self.wallet_state_manager.main_wallet.create_tandem_gl_tx(
                fee,
                tx_config,
        ...
        if gold_tx is not None and gold_tx.spend_bundle is not None:
            spend_bundles.append(gold_tx.spend_bundle)
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            gold_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_gl_tx(
                fee, tx_config, extra_conditions=(vc_announcement,)
            )
            assert did_tx.spend_bundle is not None
            assert gold_tx.spend_bundle is not None
            new_bundle = SpendBundle.aggregate([gold_tx.spend_bundle, did_tx.spend_bundle])
