...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive GL though a wallet of this type.
    ...
    # if asset_id == None: then we get normal GL
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSGoldConnection, coin_data: Optional[Any]) -> None:
        """
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                gl_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == gl_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        wallet_node: Any = self.wallet_state_manager.wallet_node
        peer: WSGoldConnection = wallet_node.get_full_node_peer()
        if peer is None:  # pragma: no cover
            raise ValueError("Could not find any peers to request puzzle and solution from")
        ...
        # get existing gl funds for treasury
        gl_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([gl_funds_puzhash], [self.id()])
    ...
    async def generate_proposal_vote_spend(
        ...
        if fee > 0:
            ...
            spend_bundle = unsigned_spend_bundle.aggregate([unsigned_spend_bundle, dao_cat_spend, gold_tx.spend_bundle])
        else:
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    gl_coins = await self.select_coins_for_asset_type(uint64(sum))
                    for gl_coin in gl_coins:
                        gl_parent_amount_list.append([gl_coin.parent_coin_info, gl_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(make_spend(gl_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            gold_tx = await self.standard_wallet.create_tandem_gl_tx(fee, tx_config)
            assert gold_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, gold_tx.spend_bundle])
        else:
    ...
    async def free_coins_from_finished_proposals(
        ...
        if fee > 0:
            gold_tx = await self.standard_wallet.create_tandem_gl_tx(fee, tx_config)
            assert gold_tx.spend_bundle is not None
            full_spend = full_spend.aggregate([full_spend, gold_tx.spend_bundle])
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "gl_conditions": gl_created_coins,
