...
def get_spend_p2_singleton_puzzle(treasury_id: bytes32, gl_conditions: Program, asset_conditions: Program) -> Program:
    # TODO: typecheck get_spend_p2_singleton_puzzle arguments
    # TODO: add tests for get_spend_p2_singleton_puzzle: pass gl_conditions as Puzzle, List and ConditionWithArgs
    #

    # CAT_MOD_HASH
    # CONDITIONS  ; GL conditions, to be generated by the treasury
...
def generate_simple_proposal_innerpuz(
    ...
    for recipient_puzhash, amount, asset_type in zip(recipient_puzhashes, amounts, asset_types):
        if asset_type:
            ...
            gl_conds.append([51, recipient_puzhash, amount])
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(gl_conds), Program.to(cat_conds))
    return puzzle
...
async def generate_mint_proposal_innerpuz(
    ...
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(gl_conditions), Program.to([]))
    return puzzle
