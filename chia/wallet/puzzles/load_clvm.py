...
recompile_requested = (
    (os.environ.get("GOLD_DEV_COMPILE_CLVM_ON_IMPORT", "") != "") or ("pytest" in sys.modules)
) and os.environ.get("GOLD_DEV_COMPILE_CLVM_DISABLED", None) is None
...
def load_serialized_clvm(
    ...
    # Set the GOLD_DEV_COMPILE_CLVM_ON_IMPORT environment variable to anything except
        ...
        if full_path.exists():
            ...
            if not output.exists() or os.stat(full_path).st_mtime > os.stat(output).st_mtime:
                ...
                if include_standard_libraries:
                    ...
                    gold_puzzles_path = pathlib.Path(__file__).parent
                    search_paths.append(gold_puzzles_path)
