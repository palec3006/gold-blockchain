...
from gold_rs import compute_merkle_set_root, confirm_included_already_hashed, confirm_not_included_already_hashed
...
async def request_and_validate_removals(
    peer: WSGoldConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSGoldConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSGoldConnection, bool]],
...
async def fetch_coin_spend(height: uint32, coin: Coin, peer: WSGoldConnection) -> CoinSpend:
...
async def fetch_coin_spend_for_coin_state(coin_state: CoinState, peer: WSGoldConnection) -> CoinSpend:
