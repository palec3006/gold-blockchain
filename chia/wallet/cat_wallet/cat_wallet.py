...
from gold_rs import G1Element, G2Element
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            gold_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        gold_tx = dataclasses.replace(gold_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        tx_list = await self.wallet_state_manager.add_pending_transactions([gold_tx, cat_record])
        return self, tx_list
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGoldConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_gl_tx(
        ...
        if fee > amount_to_claim:
            gold_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(gold_coins)[0].name()
            [gold_tx] = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            assert gold_tx.spend_bundle is not None
        else:
            gold_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            ...
            origin_id = list(gold_coins)[0].name()
            selected_amount = sum([c.amount for c in gold_coins])
            [gold_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in gold_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some GL as part of this
        regular_gold_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_gold_to_claim = payment_amount

        need_gold_transaction = (fee > 0 or regular_gold_to_claim > 0) and (fee - regular_gold_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_gold_transaction:
                    if fee > regular_gold_to_claim:
                        gold_tx, _ = await self.create_tandem_gl_tx(
                            fee,
                            uint64(regular_gold_to_claim),
                            tx_config,
                    ...
                    elif regular_gold_to_claim > fee:  # pragma: no cover
                        gold_tx, gl_announcement = await self.create_tandem_gl_tx(
                            fee,
                            uint64(regular_gold_to_claim),
                            tx_config,
                        )
                        assert gl_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            conditions=(*extra_conditions, gl_announcement, announcement),
                        )
                    else:
        ...
        gold_spend_bundle = SpendBundle([], G2Element())
        if gold_tx is not None and gold_tx.spend_bundle is not None:
            gold_spend_bundle = gold_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        spend_bundle, gold_tx = await self.generate_unsigned_spendbundle(
            payments,
            tx_config,
            fee,
        ...
        if gold_tx is not None:
            ...
            other_tx_additions: Set[Coin] = {removal for removal in gold_tx.additions}
        else:
        ...
        if gold_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=gold_tx.confirmed_at_height,
                    created_at_time=gold_tx.created_at_time,
                    to_puzzle_hash=gold_tx.to_puzzle_hash,
                    amount=gold_tx.amount,
                    fee_amount=gold_tx.fee_amount,
                    confirmed=gold_tx.confirmed,
                    sent=gold_tx.sent,
                    spend_bundle=None,
                    additions=gold_tx.additions,
                    removals=gold_tx.removals,
                    wallet_id=gold_tx.wallet_id,
                    sent_to=gold_tx.sent_to,
                    trade_id=gold_tx.trade_id,
                    type=gold_tx.type,
                    name=gold_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
