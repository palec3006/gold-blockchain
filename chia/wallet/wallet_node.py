...
from chia.protocols.wallet_protocol import (
    ...
    SendTransaction, RespondCoinRecords, RequestCoinRecords,
)
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSGoldConnection) -> PeerRequestCache:
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.glcoin.net"]),
    ...
    async def on_disconnect(self, peer: WSGoldConnection) -> None:
    ...
    async def on_connect(self, peer: WSGoldConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    async def state_update_received(self, request: CoinStateUpdate, peer: WSGoldConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSGoldConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSGoldConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSGoldConnection]:
        """
        ...
        synced_and_trusted: List[WSGoldConnection] = []
        synced: List[WSGoldConnection] = []
        trusted: List[WSGoldConnection] = []
        neither: List[WSGoldConnection] = []
        all_nodes: List[WSGoldConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSGoldConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSGoldConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSGoldConnection
    ) -> None:
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSGoldConnection) -> None:
    ...
    async def wallet_short_sync_backtrack(
            self, header_block: HeaderBlock, difficulty_coeff: Decimal, peer: WSGoldConnection
    ) -> int:
        peak: Optional[HeaderBlock] = await self.wallet_state_manager.blockchain.get_peak_block()

        top = header_block
        blocks = [(top, difficulty_coeff)]
        ...
        while not self.wallet_state_manager.blockchain.contains_block(top.prev_header_hash) and top.height > 0:
            ...
            blocks.append((prev_head, Decimal(response_prev.difficulty_coeff)))
            top = prev_head
        ...
        for block, difficulty_coeff in blocks:
            # Set blockchain to the latest peak
            res, err = await self.wallet_state_manager.blockchain.add_block(block, difficulty_coeff)
    ...
    async def fetch_and_update_weight_proof(self, peer: WSGoldConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSGoldConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSGoldConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSGoldConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
