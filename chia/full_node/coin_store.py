...
class CoinStore:
    ...
    async def get_unspent_coins_before_height(self, puzzle_hash: bytes32, height: uint32) -> List[CoinRecord]:
        coin_dict: Dict[bytes32, CoinRecord] = {}
        async with self.db_wrapper.reader_no_transaction() as conn:
            async with conn.execute(
                    "SELECT confirmed_index, spent_index, coinbase, puzzle_hash, "
                ...
                for row in await cursor.fetchall():
                    coin = self.row_to_coin(row)
                    record = CoinRecord(coin, row[0], uint32(0), row[2], row[6])
                    if record.name not in coin_dict:
                        coin_dict[record.name] = record
