...
import anyio
from gold_rs import AugSchemeMPL, G1Element, G2Element, MerkleSet
...
class FullNodeAPI:
    ...
    async def request_peers(
        self, _request: full_node_protocol.RequestPeers, peer: WSGoldConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: WSGoldConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSGoldConnection
    ) -> Optional[Message]:
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: WSGoldConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: WSGoldConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: WSGoldConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSGoldConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSGoldConnection
    ) -> Optional[Message]:
        """
        ...
        async with self.full_node.timelord_lock:
            ...
            self.log.debug(f"[debug] creating block {height} {difficulty_coeff}")
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSGoldConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSGoldConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSGoldConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSGoldConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def request_block_header(self, request: wallet_protocol.RequestBlockHeader) -> Optional[Message]:
        ...
        difficulty_coeff = await self.full_node.blockchain.get_farmer_difficulty_coeff(
            ...
            block.height - 1 if block.height > 0 else 0,
        )
        msg = make_msg(
            ProtocolMessageTypes.respond_block_header,
            wallet_protocol.RespondBlockHeader(header_block, str(difficulty_coeff)),
        )
        return msg
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: WSGoldConnection, request_bytes: bytes = b""
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @api_request(peer_required=True)
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSGoldConnection) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @api_request()
    async def request_stakings(self, request: farmer_protocol.RequestStakings) -> Message:
        stakings = []
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: WSGoldConnection
    ) -> Message:
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: WSGoldConnection
    ) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
    ...
    async def request_remove_puzzle_subscriptions(
        self, request: wallet_protocol.RequestRemovePuzzleSubscriptions, peer: WSGoldConnection
    ) -> Message:
    ...
    async def request_remove_coin_subscriptions(
        self, request: wallet_protocol.RequestRemoveCoinSubscriptions, peer: WSGoldConnection
    ) -> Message:
        peer_id = peer.peer_node_id
        subs = self.full_node.subscriptions
    ...
    async def request_puzzle_state(
        self, request: wallet_protocol.RequestPuzzleState, peer: WSGoldConnection
    ) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
        subs = self.full_node.subscriptions
    ...
    async def request_coin_state(self, request: wallet_protocol.RequestCoinState, peer: WSGoldConnection) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
        subs = self.full_node.subscriptions
    ...
    async def mempool_updates_for_puzzle_hashes(
        self, peer: WSGoldConnection, puzzle_hashes: Set[bytes32], include_hints: bool
    ) -> None:
    ...
    async def mempool_updates_for_coin_ids(self, peer: WSGoldConnection, coin_ids: Set[bytes32]) -> None:
        if Capability.MEMPOOL_UPDATES not in peer.peer_capabilities:
            return
    ...
    def max_subscriptions(self, peer: WSGoldConnection) -> int:
    ...
    def max_subscribe_response_items(self, peer: WSGoldConnection) -> int:
        if self.is_trusted(peer):
    ...
    def is_trusted(self, peer: WSGoldConnection) -> bool:
