...
from chia.cmds.init_funcs import check_keys, gold_init
...
from chia.util.chia_version import gold_short_version
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Gold.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "gold": "gold",
        "gold_data_layer": "gold_data_layer",
        "gold_data_layer_http": "gold_data_layer_http",
        "gold_wallet": "gold_wallet",
        "gold_full_node": "gold_full_node",
        "gold_harvester": "gold_harvester",
        "gold_farmer": "gold_farmer",
        "gold_introducer": "gold_introducer",
        "gold_timelord": "gold_timelord",
        "gold_timelord_launcher": "gold_timelord_launcher",
        "gold_full_node_simulator": "gold_simulator",
        "gold_seeder": "gold_seeder",
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Gold will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        if plotter == "bladebit":
            ...
            # 'gold plotters bladebit ramplot' or 'gold plotters bladebit diskplot'
            plot_type = request["plot_type"]
    ...
    async def start_service(self, websocket: WebSocketResponse, request: Dict[str, Any]):
        ...
        if not validate_service(service_command):
            error = "unknown service : " + service_command
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with GOLD_ROOT: {os.environ['GOLD_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in gold_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    gold_init(root_path, should_check_keys=(not wait_for_unlock))
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"gold-blockchain version: {gold_short_version()}")
