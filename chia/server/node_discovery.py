...
class FullNodeDiscovery:
    ...
    async def update_peer_timestamp_on_message(self, peer: WSGoldConnection) -> None:
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self) -> None:
        ...
        async def on_connect(peer: WSGoldConnection) -> None:
    ...
    async def on_connect_callback(self, peer: WSGoldConnection) -> None:
