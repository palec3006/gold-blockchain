...
class GoldSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _gold_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class GoldProactor(IocpProactor):
        ...
        async def _gold_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._gold_accept(listener)
        ...
        def _gold_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: GoldProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._gold_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class GoldProactorEventLoop(ProactorEventLoop):
...
class GoldPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = GoldProactorEventLoop
        else:
            loop_factory = GoldSelectorEventLoop
...
def set_chia_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(GoldPolicy())
