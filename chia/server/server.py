...
from chia.server.ws_connection import ConnectionCallback, WSGoldConnection
...
class GoldServer:
    ...
    all_connections: Dict[bytes32, WSGoldConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        gold_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> GoldServer:
        ...
        gold_ca_crt_path, gold_ca_key_path = gold_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=gold_ca_crt_path,
                ca_key=gold_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSGoldConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("GoldServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSGoldConnection] = None
        try:
            connection = WSGoldConnection.create(
    ...
    async def connection_added(
        self, connection: WSGoldConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    async def connection_closed(
        self, connection: WSGoldConnection, ban_time: int, closed_connection: bool = False
    ) -> None:
    ...
    async def send_to_all_if(
        ...
        messages: List[Message],
        node_type: NodeType,
        predicate: Callable[[WSGoldConnection], bool],
        exclude: Optional[bytes32] = None,
    ) -> None:
        await self.validate_broadcast_message_type(messages, node_type)
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSGoldConnection, trusted_peers: Dict[str, Any]) -> bool:
