...
from chia.server.ssl_context import gold_ssl_ca_paths, private_ssl_ca_paths
...
from ..util.chia_version import gold_short_version
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol, _T_RpcApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSGoldConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"gold-blockchain version: {gold_short_version()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        gold_ca_crt, gold_ca_key = gold_ssl_ca_paths(root_path, self.config)
        ...
        self._server = GoldServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for GoldServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"gold_{self._service_name}"
