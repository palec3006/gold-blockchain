...
ConnectionCallback = Callable[["WSGoldConnection"], Awaitable[None]]
...
class WSGoldConnection:
    ...
    async def call_api(
        ...
        self.log.debug(
            f"Time {self.local_type} request {request.type}: {sent_message_type.name} "
            f"{response.type}: {recv_message_type.name} {recv_method}"
        )
