...
def get_gold_ca_crt_key() -> Tuple[Any, Any]:
    here = importlib_resources.files(__name__.rpartition(".")[0])
    crt = here.joinpath("gold_ca.crt").read_bytes()
    key = here.joinpath("gold_ca.key").read_bytes()
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Gold"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Gold"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("glcoin.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Gold"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Gold CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    gold_ca_crt, gold_ca_key = get_gold_ca_crt_key()
    gold_ca_crt_path = ca_dir / "gold_ca.crt"
    gold_ca_key_path = ca_dir / "gold_ca.key"
    write_ssl_cert_and_key(gold_ca_crt_path, gold_ca_crt, gold_ca_key_path, gold_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./gold_ca.crt"), Path("./gold_ca.key"))
