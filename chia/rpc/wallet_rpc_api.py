...
from gold_rs import AugSchemeMPL, G1Element, G2Element, PrivateKey
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSGoldConnection, coin_id: bytes32, latest: bool = True
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `gold keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "gl")
        ...
        key = None
        if asset_id is not None and asset_id != "gl":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.GL.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        gl_coin_list = request.get("gl_coins", None)
        gl_coins = None
        if gl_coin_list:
            gl_coins = {Coin.from_json_dict(gl_coin) for gl_coin in gl_coin_list}
        gl_change_target = request.get("gl_change_target", None)
        if gl_change_target is not None:
            if gl_change_target[:2] == "gl":
                gl_change_ph = decode_puzzle_hash(gl_change_target)
            else:
                gl_change_ph = bytes32(hexstr_to_bytes(gl_change_target))
        else:
            gl_change_ph = None
        ...
        if mint_from_did:
            ...
            txs = await nft_wallet.mint_from_gl(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def _get_pool_nft_program_puzzle(
        ...
        if delayed_puzzle_hash is None:
            ...
            if contract_ph is None:
                puzzle_hashes_list: Dict[bytes32, Tuple[bytes32, Program]] = {}
                batch_size = 500
                for puzzle_hash in puzzle_hashes:
                    puzzle = create_p2_singleton_puzzle(POOL_OUTER_MOD_HASH, launcher_id, delay, puzzle_hash)
                    ph = puzzle.get_tree_hash()
                    puzzle_hashes_list[ph] = (puzzle_hash, puzzle)
                    if len(puzzle_hashes_list) >= batch_size:
    ...
    async def find_pool_nft(self, request) -> EndpointResult:
        ...
        try:
            ...
            log.debug(f"pool {delay} {launcher_id.hex()} {contract_address}")
            ...
            coin_records = await self.service.request_coin_records_by_puzzle_hash(puzzle_hashes=[contract_ph])
    ...
    async def recover_pool_nft(self, request, tx_config: TXConfig = DEFAULT_TX_CONFIG) -> EndpointResult:
        launcher_hash = request.get("launcher_id", "")
        contract_address = request.get("contract_address", "")
        fee = uint64(request.get("fee", 0))
        if not (len(launcher_hash) == 66 or len(launcher_hash) == 64):
        ...
        try:
            delay: uint64 = uint64(604800)
            launcher_id = bytes32(hexstr_to_bytes(launcher_hash))
            log.debug(f"recover_pool_nft {delay} {launcher_id.hex()} {contract_address}")
            program_puzzle = await self._get_pool_nft_program_puzzle(launcher_id, contract_address, delay)
            if program_puzzle is None:
                return {"success": False, "error": "the nft doesn't belong to you"}
                ...
                for coin_record in coin_records:
                    ...
                    if coin_record.timestamp <= uint64(time.time()) - delay:
                        ...
                        if len(coin_spends) >= 100:
                            ...
                            log.debug(f"pool wallet truncating absorb to 100 spends to fit into block")
            ...
            if len(coin_spends) > 0:
                ...
                fee_tx = None
                if fee > 0:
                    fee_tx = await self.service.wallet_state_manager.main_wallet.create_tandem_gl_tx(
                        fee,
                        tx_config,
                        extra_conditions=(
                            AssertCoinAnnouncement(asserted_id=coin_spends[0].coin.name(), asserted_msg=b"$"),
                        ),
                    )
    ...
    async def stake_info(self, request) -> EndpointResult:
        ...
        expected_prefix = self.service.config["network_overrides"]["config"][selected_network]["address_prefix"]
        if source != "":
            if source[0:len(expected_prefix)] != expected_prefix:
                raise ValueError("Unexpected Address Prefix")
            source_puzzle_hash: bytes32 = decode_puzzle_hash(source)
        else:
            source_puzzle_hash: bytes32 = self.service.wallet_state_manager.stake_address
        coin_records = await self.service.request_coin_records_by_puzzle_hash(puzzle_hashes=[source_puzzle_hash])

        balance = sum(uint64(coin.coin.amount) for coin in coin_records)
        return {
            "balance": balance,
            "address": encode_puzzle_hash(source_puzzle_hash, expected_prefix),
        }

    async def stake_from(self, request) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced before stake_from transactions")
        if not isinstance(request["amount"], int):
            raise ValueError("An integer amount is required (too many decimals)")
        wallet_id = uint32(int(request.get("wallet_id", 1)))
        amount: uint64 = uint64(request["amount"])
        ...
        coin_records = await self.service.request_coin_records_by_puzzle_hash(puzzle_hashes=[source_puzzle_hash])
        if len(coin_records) == 0:
            return {"success": False, "error": "no stake"}
        ...
        for coin in sorted_coins:
            ...
            if len(coins_json) > max_num_coins:
                raise ValueError(
                    f"stake_from transaction of {amount} mojo would use more than "
                    f"{max_num_coins} coins. Try sending a smaller amount"
                )

        log.debug(f"total_amount: {total_amount} coins_json {coins_json}")
        if total_amount == 0:
            return {"success": False, "error": "stake_from amount is 0 mojo"}
        if amount > total_amount:
            return {"success": False, "error": f"stake_from amount not more {total_amount} mojo"}
        if total_amount > 0:
            wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=Wallet)
            if address != "":
                if address[0:len(expected_prefix)] != expected_prefix:
                    raise ValueError("Unexpected Address Prefix")
                address_puzzle_hash: bytes32 = decode_puzzle_hash(address)
            else:
                address_puzzle_hash: bytes32 = await wallet.get_puzzle_hash(False)
            async with self.service.wallet_state_manager.lock:
                response: Dict = await self.create_signed_transaction({
                    "push": True,
                    "merge_spends": False,
                    "additions": [{
                        "puzzle_hash": address_puzzle_hash.hex(),
                        "amount": total_amount,
                    }],
                    "coins": coins_json,
                    "fee": 0,
