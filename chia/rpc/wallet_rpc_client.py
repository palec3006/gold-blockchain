...
class WalletRpcClient(RpcClient):
    ...
    Note that this is not the same as the peer protocol, or wallet protocol (which run Gold's
    ...
    async def nft_mint_bulk(
        ...
        gl_coins: Optional[List[Dict[str, Any]]] = None,
        gl_change_target: Optional[str] = None,
        ...
        request = {
            ...
            "gl_coins": gl_coins,
            "gl_change_target": gl_change_target,
    ...
    async def stake_info(self, source: str) -> Dict:
    ...
    async def stake_from(
        self, wallet_id: int, amount: uint64, source: str, address: str,
    ) -> TransactionRecord:
        response = await self.fetch("stake_from", {
            "wallet_id": wallet_id,
            "amount": amount,
            "source": source,
            "address": address,
