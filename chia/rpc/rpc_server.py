...
from chia.server.server import GoldServer, ssl_context_for_client, ssl_context_for_server
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> GoldServer:
        ...
        # a property so as to be read only which allows GoldServer to satisfy
        # Optional[GoldServer]
        ...
...
def default_get_connections(server: GoldServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
