...
def check_plotter(plotter: List[str], expected_output: bytes, specify_tmp: bool = True) -> None:
    with tempfile.TemporaryDirectory() as path:
        ...
        process = subprocess.Popen(
            [executable_for_service("gold"), "plotters", *plotter, *tmp_dir, "--final_dir", path],
...
@click.option("--expected-gold-version", "expected_gold_version_str", required=True)
@click.option("--require-madmax/--require-no-madmax", "require_madmax", default=True)
def test_command(expected_gold_version_str: str, require_madmax: bool) -> None:
    print("testing installed executables")
    expected_gold_version = packaging.version.Version(expected_gold_version_str)

    args = [executable_for_service("gold"), "version"]
    print(f"launching: {args}")
    gold_version_process = subprocess.run(
        args,
        capture_output=True,
        encoding="utf-8",
        timeout=adjusted_timeout(30),
    )
    assert gold_version_process.returncode == 0
    assert gold_version_process.stderr == ""

    gold_version = packaging.version.Version(gold_version_process.stdout)
    print(gold_version)
    assert gold_version == expected_gold_version, f"{gold_version} != {expected_gold_version}"

    args = [executable_for_service("gold"), "plotters", "version"]
    print(f"launching: {args}")
    ...
    # check_plotter(plotter=["madmax"], expected_output=b"Multi-threaded pipelined Gold")
    # check_plotter(plotter=["bladebit", "diskplot", "--compress", "0"], expected_output=b"\nBladebit Gold Plotter")
    ...
    args = [executable_for_service("gold"), "init"]
    print(f"launching: {args}")
    subprocess.run(
        args,
        check=True,
