...
class SyncGoldCommand(Protocol):
    def run(self) -> None: ...


class AsyncGoldCommand(Protocol):
    async def run(self) -> None: ...


GoldCommand = Union[SyncGoldCommand, AsyncGoldCommand]
...
class _CommandParsingStage:
    ...
    def initialize_instance(self, **kwargs: Any) -> GoldCommand:
...
def _generate_command_parser(cls: Type[GoldCommand]) -> _CommandParsingStage:
...
def _convert_class_to_function(cls: Type[GoldCommand]) -> SyncCmd:
...
@dataclass_transform()
def gold_command(cmd: click.Group, name: str, help: str) -> Callable[[Type[GoldCommand]], Type[GoldCommand]]:
    def _gold_command(cls: Type[GoldCommand]) -> Type[GoldCommand]:
        ...
        if sys.version_info < (3, 10):  # pragma: no cover
            ...
            wrapped_cls: Type[GoldCommand] = dataclass(  # type: ignore[assignment]
                frozen=True,
            )(cls)
        else:
            wrapped_cls: Type[GoldCommand] = dataclass(  # type: ignore[assignment]
