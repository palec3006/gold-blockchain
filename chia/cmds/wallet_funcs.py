...
def print_transaction(
    ...
    if verbose:
        ...
        gold_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {gold_amount} {name}")
...
async def send(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_fee: uint64 = uint64(int(fee * units["gold"]))  # fees are always in GL mojos
        ...
        print(f"To get status, use command: gold wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                            ...
                            if item in requests:
                                ...
                                if info.supports_did:
                                    ...
                                    royalty_asset_dict[name] = (
                                        encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.GL.hrp(config)),
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['gold']} GL, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["gold"] if summary["asset"] == "GL" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["gold"] if asset == "GL" else units["cat"]
                ...
                with filepath.open(mode="w") as file:
                    offer, trade_record = await wallet_client.create_offer_for_ids(
                        ...
                        tx_config=CMDTXConfigLoader(
                            reuse_puzhash=reuse_puzhash,
                        ).to_tx_config(units["gold"], config, fingerprint),
                    )
                    if offer is not None:
                        file.write(offer.to_bech32())
                        print(f"Created offer with ID {trade_record.trade_id}")
                        print(
                            f"Use gold wallet get_offers --id "
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_gl: str = "GL"
) -> None:
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["gold"]
        wid: str = "1" if asset_id == "gl" else ""
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['gold']} GL, {fees} mojos")
    print("---------------")
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_gl = AddressType.GL.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        await print_offer_summary(cat_name_resolver, offered, network_gl=network_gl)
        print("  REQUESTED:")
        await print_offer_summary(cat_name_resolver, requested, network_gl=network_gl)
        ...
        for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
            if royalty_asset_id.hex() in offered:
                ...
                royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                    encode_puzzle_hash(address, AddressType.GL.hrp(config)),
                    percentage,
                )
        ...
        if royalty_asset_dict != {}:
            ...
            for fungible_asset_id in fungible_assets_from_offer(offer):
                fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "gl"
                if fungible_asset_id_str in requested:
                    nft_royalty_currency: str = "Unknown CAT"
                    if fungible_asset_id is None:
                        nft_royalty_currency = network_gl
                    else:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["gold"] if summary["asset"] == network_gl else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["gold"] if asset == network_gl else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['gold']} {network_gl}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["gold"], config, fingerprint),
            )
            print(f"Accepted offer with ID {trade_record.trade_id}")
            print(f"Use gold wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["gold"], config, fingerprint), secure=secure, fee=fee
        )
        print(f"Cancelled offer with ID {trade_record.trade_id}")
        if secure:
            print(f"Use gold wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:  # pragma: no cover
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["gold"]
    return "", units["mojo"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        try:
            response = await wallet_client.create_new_did_wallet(
                amount, CMDTXConfigLoader().to_tx_config(units["gold"], config, fingerprint), fee, name
            )
            wallet_id = response["wallet_id"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                did_wallet_id,
                CMDTXConfigLoader().to_tx_config(units["gold"], config, fingerprint),
                extra_conditions=(
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = (
            ...
            else ensure_valid_address(royalty_address, allowed_types={AddressType.GL}, config=config)
        )
        target_address = (
            None
            if not target_address
            else ensure_valid_address(target_address, allowed_types={AddressType.GL}, config=config)
        )
        fee: int = int(d_fee * units["gold"])
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = ensure_valid_address(target_address, allowed_types={AddressType.GL}, config=config)
            fee: int = int(d_fee * units["gold"])
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["gold"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["gold"])

        tx = await wallet_client.send_notification(address, message, amount, fee)

        print("Notification sent successfully.")
        print(f"To get status, use command: gold wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.GL:
            if address is None:
                print("Address is required for GL address type.")
                return
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["gold"]), force)
        print(str(response))
        ...
        vc_record, txs = await wallet_client.vc_mint(
            ...
            (
                ...
                if target_address is None
                    ...
                    ensure_valid_address(target_address, allowed_types={AddressType.GL}, config=config)
                )
            ),
            uint64(int(d_fee * units["gold"])),
        )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["gold"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["gold"], config, fingerprint),
        )
...
async def find_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        total_amount = response["total_amount"] / units["gold"]
        record_amount = response["record_amount"] / units["gold"]
        balance_amount = response["balance_amount"] / units["gold"]
        print(f"Contract Address: {address}")
        print(f"Total Amount: {total_amount} GL")
        print(f"Balance Amount: {balance_amount} GL")
        print(f"Record Amount: {record_amount} GL")
...
async def recover_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        amount = response["amount"] / units["gold"]
        total_amount = response["total_amount"] / units["gold"]
        print(f"Contract Address: {address}")
        print(f"Total Amount: {total_amount} GL")
        print(f"Can claim rewards amount(older than 7 days): {amount} GL")
        print(f"Status: {status}")


async def stake_info(wallet_rpc_port: Optional[int], fp: Optional[int], source: str) -> None:
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        balance, address = await wallet_client.staking_info(source)
        amount = balance / units["gold"]
        print(f"Stake Balance: {amount} GL")
        print(f"Stake Address: {address}")


async def stake_from(
    wallet_rpc_port: Optional[int],
    fp: Optional[int],
    wallet_id: int,
    amount: str,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        print("Submitting transaction...")
        final_amount = uint64(int(float(amount) * units["gold"]))
        res = await wallet_client.stake_from(wallet_id, final_amount, source, address)
