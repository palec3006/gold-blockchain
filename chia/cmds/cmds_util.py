...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'gold wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'gold keys generate' or import a key")
        ...
        click.option(
            ...
            click.option(
                ...
                click.option(
                    ...
                    "amounts_to_exclude",
                    multiple=True,
                    help="Exclude any coins with this GL or CAT amount from being included.",
                )(func)
            )
        )
    )
