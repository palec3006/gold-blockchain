...
@wallet_cmd.command("send", help="Send gold to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much GL or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much GL or CAT units",
    type=str,
    required=False,
    default=None,
)
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in GL", default="0", show_default=True
)
@click.option(
    "--force",
...
@wallet_cmd.command("make_offer", help="Create an offer of GL/CATs/NFTs for GL/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in GL", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in GL", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Gold Wallet DBs"""
...
@click.option("-m", "--fee", help="The fee for the transaction, in GL", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in GL", type=str, required=False, default="0")
def mint_vc_cmd(
        wallet_rpc_port: Optional[int],
        fingerprint: int,
        did: str,
...
@click.option(
    "-m", "--fee", help="Blockchain fee for update transaction, in GL", type=str, required=False, default="0"
)
@click.option(
...
@click.option(
    "-m", "--fee", help="Blockchain fee for revocation transaction, in GL", type=str, required=False, default="0"
)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
...
@click.option(
    "-m", "--fee", type=str, default=0, show_default=True, help="Blockchain fee for approval transaction, in GL"
)
...
@wallet_cmd.command("find_pool_nft", short_help="fnd nft gold")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-l",
    "--launcher-id",
    help="Set the launcher id, you should get it from gold wallet",
    type=str,
    required=True
)
@click.option(
...
@wallet_cmd.command("recover_pool_nft", short_help="recover nft gold to wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@wallet_cmd.command("send_from", help="Transfer gl away from a specific puzzle hash")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-a",
    "--amount",
    help="Transfer gl amount, 0 will transfer all",
    type=str,
    default="0",
    show_default=True
)
@click.option("-s", "--source", help="Address to send the GL from", type=str, default="", required=False)
@click.option("-t", "--address", help="Address to send the GL", type=str, default="", required=False)
def send_from_cmd(
        wallet_rpc_port: Optional[int],
        fingerprint: int,
        id: int,
    ...
    from .wallet_funcs import stake_from
