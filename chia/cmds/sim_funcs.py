...
def create_gold_directory(
    ...
    """
    This function creates a new gold directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from chia.cmds.init_funcs import gold_init

    if not gold_root.is_dir() or not Path(gold_root / "config" / "config.yaml").exists():
        # create gold directories & load config
        gold_init(gold_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(gold_root, "config.yaml")
        ...
        if not docker_mode:  # We want predictable ports for our docker image.
            ...
            config["network_overrides"]["config"]["simulator0"]["default_full_node_port"] = 34444 + port_offset
            # wallet
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `gold keys show`.
    """
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    os.environ["GOLD_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create gold directory & get config.
    print("Creating gold directory & config...")
    config = create_gold_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your GOLD_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically gold show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['gold']} TGL."
        )
        if show_addresses:
            print("All Addresses: ")
