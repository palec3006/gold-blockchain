...
async def get_ph_balance(
    puzzle_hash: bytes32, rpc_port: Optional[int], root_path: Path = DEFAULT_ROOT_PATH
) -> Optional[Tuple[int, str]]:
    async with get_any_service_client(FullNodeRpcClient, rpc_port, root_path) as (client, config):
        prefix = selected_network_address_prefix(config)
        coins = await client.get_coin_records_by_puzzle_hash(puzzle_hash, False)
        return sum(coin.coin.amount for coin in coins), encode_puzzle_hash(puzzle_hash, prefix)
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total gold farmed: {amounts['farmed_amount'] / units['gold']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['gold']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['gold']}")
    ...
    if harvesters_summary is not None:
        ...
        def process_harvesters(harvester_peers_in: Dict[str, Dict[str, Any]]) -> None:
            for harvester_peer_id, harvester_dict in harvester_peers_in.items():
                ...
                if syncing is not None and syncing["initial"]:
                    ...
                    staking_addresses = dict(harvester_dict["staking_addresses"])
                    for puzzle_hash in staking_addresses:
                        PlotStats.staking_addresses[puzzle_hash] += staking_addresses[puzzle_hash]
                    print(
        ...
        for k, v in sorted(PlotStats.staking_addresses.items(), key=(lambda tup: tup[1]), reverse=True):
            balance, ph = await get_ph_balance(bytes32.from_hexstr(k), rpc_port, root_path)
            # query balance
            print(f"  {ph} (balance: {balance/10 ** 12}, plots: {v})")
    else:
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'gold start wallet' and 'gold wallet show'")
