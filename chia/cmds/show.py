...
def show_cmd(
    ...
    if connections:
        print("'gold show -c' has been renamed to 'gold peer full_node -c' ")
    if add_connection != "":
        print("'gold show -a' has been renamed to 'gold peer full_node -a' ")
    if remove_connection != "":
        print("'gold show -r' has been renamed to 'gold peer full_node -r' ")
    if wallet_rpc_port is not None:
        print("'gold show -wp' is not used, please remove it from your command.")
    asyncio.run(
        show_async(
            rpc_port,
            ctx.obj["root_path"],
