...
def print_restart_warning() -> None:
    print("\nRestart the daemon and any running gold services for changes to take effect.")
...
def configure(ctx: click.Context, path: Optional[str], interval: Optional[int]) -> None:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if "beta" not in config:
            raise click.ClickException("beta test mode is not enabled, enable it first with `gold beta enable`")
...
def prepare_submission_cmd(ctx: click.Context) -> None:
    with lock_and_load_config(ctx.obj["root_path"], "config.yaml") as config:
        ...
        if beta_root_path is None:
            raise click.ClickException("beta test mode not enabled. Run `gold beta enable` first.")
    ...
    gold_blockchain_path = Path(prepare_result / "gold-blockchain")
    gold_logs = prepare_logs(plotting_path, prepare_gold_blockchain_log)
    plotting_logs = prepare_logs(gold_blockchain_path, prepare_plotting_log)
    ...
    with zipfile.ZipFile(submission_file_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
        files_added = add_files(gold_logs) + add_files(plotting_logs)

    if files_added == 0:
        submission_file_path.unlink()
        message = f"No logs files found in {str(plotting_path)!r} and {str(gold_blockchain_path)!r}."
