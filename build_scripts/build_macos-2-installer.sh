...
LAST_EXIT_CODE=$?
ls -l dist/mac*/gold.app/Contents/Resources/app.asar
...
DMG_NAME="gold-${GOLD_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/gold-"${GOLD_INSTALLER_VERSION}"-arm64.dmg
  DMG_NAME=gold-${GOLD_INSTALLER_VERSION}-arm64.dmg
fi
...
# xcrun notarytool submit --wait --apple-id username --password password --team-id team-id Gold-0.1.X.dmg
# Wait until the command returns a success message
#
# Once that is successful, execute the following command":
# xcrun stapler staple Gold-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Gold-0.1.X.dmg
