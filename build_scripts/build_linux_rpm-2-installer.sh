...
fpm -s dir -t rpm \
  ...
  --name gold-blockchain-cli \
  --license Apache-2.0 \
  --version "$GOLD_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Gold is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
# sets the version for gold-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$GOLD_INSTALLER_VERSION" '.version=$VER' package.json >temp.json && mv temp.json package.json
...
echo npx electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.extraMetadata.name=gold-blockchain \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Gold Blockchain" \
  --config.rpm.packageName="gold-blockchain" \
...
GUI_RPM_NAME="gold-blockchain-${GOLD_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${GOLD_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
